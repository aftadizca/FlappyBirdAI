...
App.Main.prototype = {
	preload : function(){
		this.game.load.spritesheet('imgBird', 'assets/img_bird.png', 48, 36, 20);
		...
		this.load.bitmapFont('fnt_chars_black', 'assets/font.png', 'assets/font.fnt');
	...
	create : function(){
		...
		// set warna background game
		this.game.stage.backgroundColor = "#42A5F5";
		
		// tetap jalan jika hilang fokus
		this.game.stage.disableVisibilityChange = true;
		
		// starting Phaser arcade physics engine
		this.game.physics.startSystem(Phaser.Physics.ARCADE);

		// set gravitasi
		this.game.physics.arcade.gravity.y = 1300;
		
		// buat Genetic Algorithm dengan populasi 10 dan 4 teratas yg akan berkembang biak
		...
		this.txtBestUnit = new Text(this.game, 1095, 620, "", "center","fnt_chars_black");
		
		// create buttons
		//this.btnRestart = this.game.add.button(1040, 620, 'imgButtons', this.onRestartClick, this, 0, 0);
		this.btnRestart = new LabelButton(this.game,100, 690,"btn", "RESTART", this.onRestartClick, this,  5, 3, 4, 3);
		this.btnPause = new LabelButton(this.game,300, 690,"btn", "PAUSE", this.onPauseClick, this,  5, 3, 4, 3);
		this.btnPause = new LabelButton(this.game,500, 690,"btn", "MAIN MENU", this.onMainMenuClick, this,  5, 3, 4, 3);
	...
	drawStatus : function(){
		...
		this.bmdStatus.fill(0, 150, 136); // clear bitmap data by filling it with a green color
		this.bmdStatus.rect(0, 0, this.bmdStatus.width, 35, "#004D40"); // draw the HUD header rect
			
		this.BirdGroup.forEach(function(bird){

			var y = 85 + bird.index*50;
								
			this.bmdStatus.draw(bird, 28, y-25); // draw bird's image
	...
	onMainMenuClick : function(){
		...
		game.state.add('Menu', App.Menu);
		game.state.start('Menu');
		//localStorage.setItem("trainingData",this.GA);
	},
